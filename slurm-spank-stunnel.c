...
// 		ERROR("Warning: The hostname file %s exists and will be
// overwritten.  There may stray ssh processes that should be killed.",
// filename);
// 	}
//
...
//
// /*
//  * Reads the host file so that the ssh tunnel can be terminated.  Deletes
//  the
...
// 	if ( file == NULL ) {
// 		// ERROR("tunnel: unable to read file %s. You may need to
// manually kill ssh tunnel processes.", filename); 		return 30;
// 	}
//
...
int _connect_node(spank_t spank, char* node) {
    ...
    if (file_exists(tunnel_controlfile)) {
        ERROR(
            "STUNNEL=> ssh control file %s already exists. Either you "
            "already have "
            "a tunnel in place, or one did not terminate correctly. Please "
            "remove "
            "this file.",
            tunnel_controlfile);
        exit(1);
    ...
    expc_length = strlen(node) + strlen(((ssh_cmd == NULL) ? DEFAULT_SSH_CMD : ssh_cmd)) +
                  strlen(((ssh_args == NULL) ? DEFAULT_SSH_ARGS : ssh_args)) + strlen(args) +
                  strlen(tunnel_controlfile) + strlen(helpertask_args) + 20;
    expc_cmd = (char*) malloc(expc_length * sizeof(char));
    if (expc_cmd != NULL) {
        snprintf(expc_cmd, expc_length, expc_pattern, (ssh_cmd == NULL) ? DEFAULT_SSH_CMD : ssh_cmd,
                 (ssh_args == NULL) ? DEFAULT_SSH_ARGS : ssh_args, node, args, tunnel_controlfile,
...
// int slurm_spank_user_init (spank_t spank, int ac, char **av) //root ?
{
    ...
    if (spank_getenv(spank, STUNNEL_MODE, tunnel_mode, sizeof(tunnel_mode)) == ESPANK_SUCCESS) {
        ...
        if (strcmp(tunnel_mode, "0") == 0) {
            DEBUG(
                "STUNNEL=> Tunnels are started locally from submission host, "
                "skip");
            return 0;
        }
    } else {
 ...
 * 				which should habe permission to terminate the
 *tunnel for the user
 *
 */
...
int close_tunnel(spank_t spank) {
    ...
    if (expc_cmd != NULL &&
        (snprintf(expc_cmd, expc_length, expc_pattern, (ssh_cmd == NULL) ? DEFAULT_SSH_CMD : ssh_cmd,
                  (ssh_args == NULL) ? DEFAULT_SSH_ARGS : ssh_args, tunnel_host, tunnel_controlfile) >= expc_length)) {
...
static int _tunnel_opt_process(int val, const char* optarg, int remote) {
    ...
    for (i = 0; i < numpairs; i++) {
        ...
        if (hoststr == NULL) {
            ERROR(
                "STUNNEL=> --tunnel parameter needs two numeric ports "
                "separated by a "
                "colon");
            free(portpairs);
            exit(1);
        ...
        if (first == 0 || second == 0) {
            ERROR(
                "STUNNEL=> --tunnel parameter requires two numeric ports "
                "separated "
                "by a colon");
            free(portpairs);
            exit(1);
        ...
        if (first < 1024 || second < 1024) {
            ERROR(
                "STUNNEL=> --tunnel cannot be used for privileged ports (< "
                "1024)");
            free(portpairs);
            exit(1);
        }
...
struct spank_option spank_opts[] = {
    {"tunnel", "<submit port:exec port[,submit port:exec port,...]>",
     "Forward exec host port to submit host port via ssh -L", 1, 0, (spank_opt_cb_f) _tunnel_opt_process},
